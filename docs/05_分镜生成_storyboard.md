...
### 核心原则
**shotPrompt 描述的是动作开始前的静止状态，shotAction 描述的是完整动作过程**

### 校验检查清单
对于每个分镜，必须满足：
1. 首帧中的所有角色在动作序列中都有对应动作
2. 动作序列中的所有关键物体在首帧中都有体现
3. 首帧的景别与运镜起始状态一致
4. 动作的起止点与镜头时长匹配

### 常见错误示例
❌ 错误：首帧是咖啡杯特写，动作是苏念端着杯子走向林晨
- 问题：首帧只有杯子，动作涉及人物移动，不匹配

✅ 修正：首帧是苏念手端咖啡杯站在吧台后近景，动作是苏念端着杯子走向林晨

❌ 错误：首帧是苏念站在吧台后，动作是苏念走向林晨并递咖啡
- 问题：首帧缺少林晨在场

✅ 修正：首帧是苏念站在吧台后手拿咖啡杯，林晨站在吧台前，动作是苏念走向林晨并递咖啡

### 验证要求
- 生成分镜时必须检查每个镜头的一致性
- 不一致的分镜应重新生成首帧描述或动作描述
- 确保 videoPrompt 能够正确引用首帧状态和动作序列
...
| 场景类型 | 推荐时长范围 | 速度建议 | 示例 |
|---------|-------------|---------|------|
| **开场/环境建立** | 4-6秒 | 缓慢 | 店内全景展示 |
| **对话场景** | 3-5秒（每句台词） | 正常 | 日常对话 |
| **情绪特写** | 2-4秒 | 较快/极快 | 眼神变化 |
| **动作过渡** | 3-5秒 | 正常 | 走位、拿取物品 |
| **高潮时刻** | 5-8秒 | 缓慢 | 重要表情/回眸 |
| **结尾/过渡** | 4-6秒 | 缓慢 | 人物离场 |

**时长总和控制**：
- 单集总时长建议控制在 60-90 秒（适应短视频平台）
- 高密度对话可扩展至 120 秒
- 根据动作复杂度动态调整时长
