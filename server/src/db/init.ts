...
- **风格锁定**: 必须严格按照用户提供的"风格"参数决定角色的整体视觉风格。**默认采用动漫风格,必须使用赛璐璃上色、清晰勾线、扁平化阴影、大眼比例等动漫特征。禁止在用户指定动漫风格时使用写实/照片级/真实等描述。**
...
4. **风格锁定**: 必须严格按照用户提供的"风格"参数决定场景的整体视觉风格。动漫风格应使用赛璐璃上色、清晰勾线、扁平化阴影、二维动漫渲染质感、大眼比例等动漫特征。禁止在用户指定动漫风格时使用写实/照片级/真实等描述。
5. **空间合理性**: 必须确保场景空间布局合理,门、窗、墙壁、家具的位置关系符合基本建筑逻辑,避免"门在窗子上"等诡异布局。
...
### 必须避免的空间布局错误
1. **门窗冲突**：门与窗在同一平面墙面上重叠
2. **穿透错误**：门、窗穿透承重结构或无法合理布置
3. **家具悬空**：大型家具（如桌子、柜台）无支撑结构
4. **透视错误**：视角与空间结构不符（如从室外看却看到室内布局）
5. **比例失调**：家具尺寸与房间大小不匹配

### 场景描述必须包含的空间要素
- **墙体结构**：明确墙壁的位置和数量
- **门窗位置**：门窗在墙面上的具体位置（左侧、右侧、中央）
- **家具布局**：主要家具的位置和朝向
- **空间连接**：房间之间的连接关系（通过门、走廊等）

### 场景示例 - 合理布局
"动漫风格，赛璐璃上色，清晰勾线。平视斜侧45度视角。温馨的咖啡店室内场景。长方形房间，左侧为深棕色木质墙壁，右侧有落地玻璃窗占据墙面中央。后墙中央为木质吧台，吧台上有咖啡机和绿植。前景为原木色桌椅，呈L形排列。阳光从右侧落地窗斜照入室内，在地面形成明亮光斑。整体色调暖黄，营造温馨舒适的氛围。"

### 场景示例 - 错误布局
"动漫风格。咖啡店室内。左侧墙面有一扇落地窗，墙上还有一扇门位于窗户中央。吧台在门和窗之间。阳光从所有方向照入。"（错误：门在窗子上，空间布局不合理）
